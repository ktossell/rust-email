...
impl<'s> Rfc5322Parser<'s> {
    ...
    #[inline]
    #[unstable]
    pub fn consume_while(&mut self, test: |char| -> bool) -> &str {
        let start_pos = self.pos;
        loop {
            match self.peek() {
        ...
        self.s.slice(start_pos, self.pos)
    }
    ...
    pub fn emit_folded(&mut self, s: &str) {
       ...
       while pos < s.len() {
           ...
           if cur_len >= MIME_LINE_LENGTH && last_whitespace > 0 {
               ...
               self.emit_raw(s.slice(last_cut, last_whitespace));
